...
- Node.js 18+
- npm 9+
- (opcional) Yarn 1.x para ambientes que utilizam `yarn install`

## Instalação rápida

Instale todas as dependências do frontend e backend de uma só vez executando na raiz do repositório:

```bash
npm install
```

Esse comando é especialmente útil em plataformas de deploy (por exemplo, Render) que executam `yarn install` automaticamente, pois o `package.json` na raiz redireciona a instalação para os diretórios `server/` e `workshop-app/`.
